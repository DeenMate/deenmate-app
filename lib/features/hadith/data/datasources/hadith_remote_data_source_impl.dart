...
class HadithRemoteDataSourceImpl implements HadithRemoteDataSource {
  ...
  Future<List<HadithCollection>> getCollections() async {
    try {
      final response =
          await _dio.get('${HadithRemoteDataSource.baseUrl}/collections');
  ...
  @override
  Future<List<Hadith>> getHadithsByCollection(String collectionId,
      {int page = 1, int limit = 20}) async {
    try {
      final response = await _dio.get(
        '${HadithRemoteDataSource.baseUrl}/collections/$collectionId/hadiths',
        queryParameters: {
          'page': page,
          'limit': limit,
        },
  ...
  Future<Hadith?> getHadithById(String hadithId) async {
    try {
      final response = await _dio.get('${HadithRemoteDataSource.baseUrl}/hadiths/$hadithId');
  ...
  @override
  Future<HadithSearchResult> searchHadiths(
    String query, {
    String? collection,
    String? grade,
    List<String>? topics,
    ...
    try {
      ...
      final response = await _dio.get(
        '${HadithRemoteDataSource.baseUrl}/hadiths/search',
      ...
      if (response.statusCode == 200) {
        ...
        final hadiths =
            hadithsData.map((json) => _mapHadithFromApi(json)).toList();
  ...
  @override
  Future<List<Hadith>> getHadithsByTopic(String topic,
      {int page = 1, int limit = 20}) async {
    try {
      final response = await _dio.get(
        '${HadithRemoteDataSource.baseUrl}/hadiths',
        queryParameters: {
          'topic': topic,
          'page': page,
          'limit': limit,
  ...
  Future<List<String>> getTopics() async {
    try {
      final response = await _dio.get('$baseUrl/topics');
  ...
  Future<List<String>> getGrades() async {
    try {
      final response = await _dio.get('$baseUrl/grades');
  ...
  Future<String?> getHadithAudioUrl(String hadithId, String language) async {
    try {
      final response = await _dio.get('$baseUrl/hadiths/$hadithId/audio');
  ...
  Future<String?> getHadithExplanation(String hadithId, String language) async {
    try {
      final response = await _dio.get('$baseUrl/hadiths/$hadithId/explanation');
  ...
  List<String> _extractBooksBengali(Map<String, dynamic> json) {
    ...
    if (books != null) {
      return books
          .map((book) => book['name_bn'] as String? ?? book['name'] as String)
          .toList();
    }
    return [];
  }
  ...
  List<String> _extractTopicsBengali(Map<String, dynamic> json) {
    ...
    if (topics != null) {
      return topics
          .map(
              (topic) => topic['name_bn'] as String? ?? topic['name'] as String)
          .toList();
    }
    return [];
  }
